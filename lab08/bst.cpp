...
// constructor
// FΙXМΕ: make the tree start out empty
BST::BST() : root(nullptr){}

// This is the insert function
// FΙXМΕ: add a new BSTNode (created on the heap) into the current tree
...
void BST::catchPokemon(const Pokemon& p) {
  ...
  if (root == nullptr) {
    root = new BSTNode{key, p, nullptr, nullptr, nullptr};
    return;
  }
  ...
  BSTNode* newNode = new BSTNode{key, p, nullptr, nullptr, parent};
...
// FΙXМΕ: search for a node by key
...
// FΙXМΕ: return the predecessor of a given node
...
// FΙXМΕ: use the search and remove functions to delete a key if it exists
...
void BST::remove(BSTNode* n) {
  ...
  std::swap(n->key, pred->key);
  std::swap(n->data, pred->data);
